...
async def insert_into_daily(user_id, strike, coins_to_give):
    await database.execute("""INSERT INTO casino_players(user_id, money, take_daily, daily_strike)
                                VALUES(?, ?, 1, ?)
                                ON CONFLICT (user_id) DO
                                UPDATE SET take_daily = excluded.take_daily;""", (user_id, coins_to_give, strike))


async def insert_into_user_money(user_id, money):
    await database.execute("""INSERT INTO casino_players(user_id, money)
                              VALUES(?, ?)
                              ON CONFLICT (user_id) DO
...
async def add_jackpot_tickets(user_id, tickets):
    await database.execute("""INSERT INTO jackpot(user_id, tickets)
                                VALUES(?, ?)
                                ON CONFLICT (user_id) DO
...
async def fetch_top_three_players():
    try:
        top_users = await database.fetch_data("""SELECT user_id, money FROM casino_players
...
async def fetch_user_money(user_id):
    try:
        data = await database.fetch_data("""SELECT money FROM casino_players
                                             WHERE user_id = ?;""", (user_id,))
        user_money = data[0][0]
    except IndexError:
        user_money = 0
...
async def fetch_tickets_number():
    data = await database.fetch_data("""SELECT SUM(tickets) FROM jackpot;""")
    return data[0][0]


async def fetch_user_tickets(user_id):
    try:
        data = await database.fetch_data("""SELECT tickets FROM jackpot
                                             WHERE user_id = ?;""", (user_id,))
...
async def fetch_all_jackpot_data_to_make_draw():
    data = await database.fetch_data("""SELECT user_id, tickets FROM jackpot""")
    return data
